...
Create a small beta artifact. Execute only after explicit PM enrollment.

## Revision and starting point

Requirements revision: 1. Repository: `OWNER/FIXTURE_B`.
Starting commit: `START_COMMIT`.

## Acceptance criteria

- Create `fixtures/beta.json` containing `{"name":"beta","value":25}`.
- Create `tests/beta.test.mjs` using Node's built-in test and assertion modules
  to read the artifact and assert its exact object.
- Run `node --test tests/beta.test.mjs` and report the actual result.
