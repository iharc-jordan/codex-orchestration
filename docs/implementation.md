...
pilot was accepted on 0.1.6 after source integration, passing CI, and live
deployment verification. Public installation and publication receipts are
recorded in the versioned GitHub release notes.
...
The plugin is Apache-2.0. The bounded pilot passed. The plugin package carries
...
are checked from a clean public Git installation before publishing the plugin
release; its release notes record that result.
...
| Restart and interruption recovery | PASS RECORDED; 0.1.6 pilot ACCEPTED | A pending requirements revision survived an owned service-process stop; the same App Server thread resumed with a new recovery turn, one checkpoint, one completion, exact-once filesystem/commit effects, and no rewind. Exact duplicate control submission was idempotent. The qualified source gate covers controls-first recovery, current-intent reread, retry/block reset, and ordered obsolete-intent retirement. |
| Bounded pilot | ACCEPTED | A fresh Astra PM used the installed bridge and 0.1.6 service to deliver one bounded production assignment through a Luna/xhigh worker, source integration, local tests, CI, live deployment verification, Project acceptance, and issue closure. |
| Public release and upstream submissions | VERSIONED RECEIPTS | See the public plugin and runtime release notes for installation verification and publication receipts. The Projects and hook-context contributions use independent upstream PRs; upstream merge is outside release acceptance. |
...
installation/start. The bounded PM pilot reached acceptance after local tests,
source integration, CI, and live deployment verification. It used one successful
Luna/xhigh implementation turn without implementation rework or escalation.
Successful dispatch to acceptance took 10 minutes 39 seconds; the full pilot
took 57 minutes 3 seconds including runtime-repair holds. This single pilot is
readiness evidence, not a percentage cost-saving estimate.

Usage telemetry can arrive late: the managed snapshot recorded 677,983 worker
tokens, while the final worker session recorded 733,723. The PM snapshot
recorded 9,527,131 raw tokens, including repeatedly cached input. These are
telemetry counts, not billed cost, and the managed snapshot is not a hard cap
on in-flight or late-reported usage. Keep provider configuration, issue content,
credentials, and detailed receipts private.
...
| Trusted managed checkout | Prepare enrolled repositories at a pinned commit while preserving safe reusable workspaces. | Managed checkout only; generic `hooks.after_create` behavior remains available outside this profile. | Helper policy tests and installed four-assignment workflow; the 0.1.6 pilot reached acceptance. | Retire after upstream provides equivalent enrollment, ancestry, context, and argument-array safeguards. |
| Configured host capability pass-through | Allow the MVP worker turn to use the capabilities configured by the host while keeping assignment scope and PM authorization explicit. | The managed workflow passes through configured CLI, MCP tools, apps, skills, network, and Docker capabilities; it does not add a second scheduler or recursive worker route. | Configuration and worker-permission checks verify the declared capabilities, trusted PM controls, resource boundaries, and recursive-delegation rejection. | Retire after the upstream managed profile provides equivalent configured-host capability and authorization behavior. |
