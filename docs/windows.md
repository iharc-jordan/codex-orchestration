...
Version 0.3.0 supports Windows 11 x64 only. It does not start an alternate
...
Use a release-manifest.json that pins repository iharc-jordan/symphony, version
0.3.0, an HTTPS GitHub v0.3.0 runtimeDownloadUrl, and its SHA-256. Alternatively
provide an offline ZIP with the same expected digest through --sha256. The
